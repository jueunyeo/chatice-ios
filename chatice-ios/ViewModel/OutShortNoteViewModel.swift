...
/*
 ...
 class OutShortNoteViewModel: ObservableObject {
     @Published var outShortNote: [ShortNote] = []
     
     let db = Firestore.firestore()
     let storage = Storage.storage()
     
     func loadShortNote(userId: String) {
         self.db.collection(K.ShortNote.collectionName).whereField(K.ShortNote.senderField, isEqualTo: userId).order(by: K.ShortNote.dateField).addSnapshotListener {(querySnapshot, error) in
             self.outShortNote = []
             if let e = error {
                             ...
                             self.db.collection(K.UserInfo.collectionName).getDocuments { (querySnapshot, error) in
                                 ...
                                 if let e = error {
                                     ...
                                     if let snapshotDocuments = querySnapshot?.documents {
                                         ...
                                         self.outShortNote.append(newShortNote)
                                     }
                                 }
                             }
                         }
